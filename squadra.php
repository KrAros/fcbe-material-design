<?php
	...
	if ($_SESSION['valido'] == "SI") {
		...
		##############################################################################
		##### Controlla se è stata inviata la formazione o se si è scelto di azzerarla
		
		if (isset($cambia_formazione)) {
			invia_formazione();
		} 
		if (isset($reset_formazione)) {
			unlink($percorso_cartella_dati."/squadra_".$outente);
		}
		
		#####################################################################################
		##### SCRIPT - Controlla il checking dei buttons radio per la posizione del giocatore
		...
		############################################################################
		##### Sezione relativa al comportamento del file nel caso di giornata APERTA
					...
					if ($otorneo == $_SESSION['torneo']) {
						...
						foreach($panchina_img as $valore_panca){
							...
							if ($riga_dati_calciatori >= 0) {
								...
								if ($valore_panca == $mnumero) {
									...
									$layout_panchina .= "<img style='border-radius: 12px;' class='z-depth-2' src=\"$m_squadra\" vspace=\"1\" hspace=\"1\" alt=\"$msquadra\" title=\"$msquadra\" /><br /><div class=\"panchinari\">$mnome</div>";
									$conta_panca ++;
							...
							if ($proprietario == $outente) {
								...
								if ($ok_pre == "SI") {
									...
									if ($dati[$dcs]['pan'] != "") {
										...
										$stat_squadra = "<i class='tiny material-icons tooltipped' data-position='top' data-tooltip='Prossima gara: $ss_part<br><p style=\"text-align:left\">Titolari: $ss_tito<br>Panchinari:$ss_panc<br>Squalificati: $ss_squa<br>Indisponibili: $ss_indi</p>' style='vertical-align: text-bottom;'>info</i>";
										} else {
								...
								if ($sec_restanti < 1 or ($stato_mercato != "I" and $stato_mercato != "P")) {
									...
									<td style='padding-left: 25px'><img class='iconasquadra z-depth-2' src='./immagini/m_".strtolower($dcs). ".gif'/> $dcs </td>
									...
									if ($outente == $_SESSION['utente']) {
										...
										${$nome_linea}[$numero] = ${$nome_linea}[$numero]."<td class='center' bgcolor='$bgtabtitolari'><label><input class='with-gap' type='radio' name='schierato$numero' value='titolare' /><span style='padding-left: 20px;'></span></label></td>
										<td style='width:15%;padding: 0;' class='center' bgcolor='$bgtabpanchinari'>
										<div class='input-field col m6' style='margin-top: 3px;margin-bottom: 0;'><label><input class='with-gap' type='radio' name='schierato$numero' value='panchinaro' /><span style='padding-left: 20px;'></span></label></div>
										<div class='input-field col m6' style='margin-top: 0px;margin-bottom: 0;'><select name='panchinaro$numero' onChange=setCheckedValue(document.seleziona_squadra.schierato$numero,'panchinaro') >\n";
										...
										<td class='center'><label><input class='with-gap' type='radio' name='schierato$numero' value='fuori' onClick=ResetOption(document.seleziona_squadra.panchinaro$numero,'-') /><span style='padding-left: 20px;'></span></label></td>";
										...
										if ($mercato_libero == "SI") {
											...
											$offri = "<td align='center'><a href='stat_calciatore.php?num_calciatore=$numero' class='user'>Statistiche</a></td>";
						...
						if ($outente == $_SESSION['utente']) {
							...
							if ($xsquadra_ok != "SI") {
								...
								<button type='submit' class='btn waves-effect waves-light green' name='cambia_formazione' id='cambia_formazione'/>Conferma formazione<i class='material-icons right'>check</i></button>
								<span style='padding:30px'></span>
								<button type='submit' class='btn waves-effect waves-light red' name='reset_formazione' id='reset_formazione'/>Azzera formazione<i class='material-icons right'>close</i></button>
								</div>";
						...
						##########################################################
						##### LAYOUT - Tabella centrale: schiera la tua formazione
						
						$tab_centro = "<div class='row'>
						<div class='col m12'>
						<div class='card' id='result'>
						<div class='card-content'>
						<span class='card-title'>Schiera la tua formazione</span>
						<hr>
						<table class='responsive-table highlight' style='width:100%' cellpadding='10' cellspacing='0' id='t1'>
						<tr>
						<thead>
						<th class='center'></th>
						<th class='left'>Giocatore</th>
						...
						if ($outente == $_SESSION['utente']) {
							$tab_centro .= "<th class='center'>Titolare</th>
							<th class='center'>Panchina</th>
							<th class='center'>Tribuna</th>";
							$colspan = 11;
						} 
						
						$tab_centro .= "<th class='center'></th>
						</tr>
						</thead>";
						
						##################################################
						##### LAYOUT - Tabella centrale: titolari in campo
						
						$tab_centro .= "<tr>
						<th class='center' colspan='$colspan'><b>TITOLARI</b></th>
						</tr>";
						...
						for($num2 = 0; $num2 < $num_titolari; $num2 ++) {
							...
							if ($linea_comprato_P[$numero_titolare]) {
								...
								$linea_comprato_P[$numero_titolare] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='portiere'>$num_pos</td>", $linea_comprato_P[$numero_titolare]);
								$num_pos ++;
							...
							if (isset($linea_comprato_D[$numero_titolare])) {
								...
								$linea_comprato_D[$numero_titolare] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_D[$numero_titolare]);
								$num_pos ++;
							...
							if (isset($linea_comprato_C[$numero_titolare])) {
								...
								$linea_comprato_C[$numero_titolare] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_C[$numero_titolare]);
								$num_pos ++;
							...
							if (isset($linea_comprato_F[$numero_titolare])) {
								...
								$linea_comprato_F[$numero_titolare] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_F[$numero_titolare]);
								$num_pos ++;
							...
							if (isset($linea_comprato_A[$numero_titolare])) {
								...
								$linea_comprato_A[$numero_titolare] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_A[$numero_titolare]);
								$num_pos ++;
						...
						##################################################
						##### LAYOUT - Tabella centrale: elenco panchinari
						
						$tab_centro .= "<tr>
						<th class='center' colspan='$colspan'><b>PANCHINA</b></th>
						</tr>";
						...
						for($num2 = 0; $num2 < $num_panchinari; $num2 ++) {
							...
							if ($linea_comprato_P[$numero_panchinaro]) {
								...
								$linea_comprato_P[$numero_panchinaro] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='portiere'>$num_pos</td>", $linea_comprato_P[$numero_panchinaro]);
								$num_pos ++;
							...
							if (isset($linea_comprato_D[$numero_panchinaro])) {
								...
								$linea_comprato_D[$numero_panchinaro] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_D[$numero_panchinaro]);
								$num_pos ++;
							...
							if (isset($linea_comprato_C[$numero_panchinaro])) {
								...
								$linea_comprato_C[$numero_panchinaro] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_C[$numero_panchinaro]);
								$num_pos ++;
							...
							if (isset($linea_comprato_F[$numero_panchinaro])) {
								...
								$linea_comprato_F[$numero_panchinaro] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_F[$numero_panchinaro]);
								$num_pos ++;
							...
							if (isset($linea_comprato_A[$numero_panchinaro])) {
								...
								$linea_comprato_A[$numero_panchinaro] = preg_replace("#<tr><td>&nbsp;</td>#", "<tr><td class='numero'>$num_pos</td>", $linea_comprato_A[$numero_panchinaro]);
								$num_pos ++;
						...
						#####################################################
						##### LAYOUT - Tabella centrale: giocatori in tribuna
						
						$tab_centro .= "<tr>
						<th class='center' colspan='$colspan'><b>TRIBUNA</b></th>
						</tr>";
						...
						##############################################################################
						##### LAYOUT - Parte superiore: doppia colonna con info giocatore e fantacampo
						...
						<div class='col m5'>												    
						<div class='card'>							    						    
						<span class='card-title white-text' style='background-color: #3f51b5;background-image:url($logo_squadra);background-repeat:no-repeat;background-position:right;background-size:40%;height:116px;padding: 42px 0 0 10px;';>Profilo giocatore</span>                                							
						<div class='card-content'>
						...
						if (file_exists($percorso_cartella_dati."/squadra_$outente")) {
							...
							$dati_utente .= "Non ancora inviata";
						}
						...
						#########################################
						##### LAYOUT - Scheletro dell'intero file
						...
						if ($outente == $_SESSION['utente']) {
							echo "<span class='card-title'>Gestione ".$_SESSION['squadra']."<span style='font-size: 13px;'> - Schiera la tua formazione</span></span>
							<hr>";
							...
							if ($tab_lato != "" and $xsquadra_ok != "SI") {
								echo "<div class='row'>
								<div class='col m12'>
								<div class='evidenziato'>$info_rosa</div>
								</div>
								</div>";
							}
							echo "$dati_utente";
							...
							<form method='post' action='' name='seleziona_squadra'>
							$tab_centro
							$fuori_tabella
							</form>
							
							<form method='post' action='' name='reset_form' id='reset_form'></form>
